...
本書は kg-wiki の検索戦略の唯一の規範である（方式設計 02 §6.4 / 基本設計 03 §5.3）。
...
| クエリ特性 | 第一手 | 続手 |
|---|---|---|
| 固有名詞・既知キーワード | `kg search <query>` | ヒット薄なら `kg vsearch`（Phase 1 では語彙を変えた `kg search` の反復で代替） |
| 「A と B の関係は」 | `kg path <ref1> <ref2>` | 経路上のページを Read |
| 概念の周辺・派生を知りたい | `kg traverse <ref> --hops 2` | `--rel` フィルタで絞る |
| 曖昧・意味的な質問 | `kg vsearch`（Phase 1 では無効。語彙を変えた `kg search` を反復） | `kg hybrid`（Phase 2） |
| 分野の俯瞰・要約 | `kg community`（Phase 1 では `_derived/index.md` の俯瞰 + `kg traverse --hops 2` で代替） | 要約から個別ページへ |
| 複合・調査型 | `kg pack`（Phase 3。Phase 1 では検索 + Read で代替） | kg-researcher へ委譲 |

- 既定の第一手は常に `kg search`（軽量機能を優先する。02 §6.4）。
